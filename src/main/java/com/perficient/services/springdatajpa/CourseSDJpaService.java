...
public class CourseSDJpaService implements CourseService {
  ...
  //Utlizar List
  //Las validacioes en los dtos son de negocio
  //retorne de unavez
  @Override
  ...
  public Page<CourseDTO> getPaginatedCourses(int numeroPagina, int tamanoPagina) {
    Pageable pageable = PageRequest.of(numeroPagina, tamanoPagina, Sort.by("name").ascending());
  ...
  public CourseDTO convertToDTO(Course course) {
    try{
      validate(course);
      CourseDTO courseDTO = CourseDTO.builder().name(course.getName()).description(course.getDescription()).credits(course.getCredits()).area(Area.DEVELOPMENT).build();
      return courseDTO;
  ...
  /*
  public Page<Course> obtenerCursosPaginados(int numeroPagina, int tamanoPagina) {
    Pageable pageable = PageRequest.of(numeroPagina, tamanoPagina, Sort.by("name").ascending());
